...
{
  memset(m_id, 0, MAX_DEVICE_ID_LENGTH + 1); // one more to ensure we have termination!
  if (m_fram == 0)
  {
  ...
  char nibble;
  int index = 0;
  while (index < MAX_DEVICE_ID_LENGTH && (nibble = m_fram->read8(index)) != '\0')
  {
    m_id[index++] = nibble;
  }
  if (index >= MAX_DEVICE_ID_LENGTH)
  {
...
void Assets::setDeviceId(const char* id){
  strncpy(m_id, id, MAX_DEVICE_ID_LENGTH);
  if (m_fram == 0)
  {
  ...
  char nibble;
  int index = 0;
  while (index < MAX_DEVICE_ID_LENGTH && (nibble = id[index]) != '\0')
  {
